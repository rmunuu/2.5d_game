...
class Screen:
    ...
    def makescreen(self):
        ...
        for npc in self.npcs:
            if -9 <= npc.coorx - self.posx <= 1 and -9 <= npc.coory - self.posy <= 1:
                npc.talk()
...
class Human:
    ...
    def showing_bag(self):
        ...
        for i, item in enumerate(self.bag):
            l = i % 13
            r = i // 13
            screen.pos_absolute_center(item.image, self.stx + 55 * l + 55 // 2 - item.image.get_width()/2, self.sty + 55 * r + 55 // 2 - item.image.get_height()/2, 0)
        human.get_item()
...
class NPC(ImageEnvironment):
    def __init__(self, img, a, b, str, d=dict(), next_str=str):
    ...
    def activate(self, x, y):
        for env in envs:
            if self.name in env.dic.keys() and env.item_activate(x, y):
                if env.dic[self.name] not in human.bag:
...
class Key(Item):
    ...
    def __init__(self, a, b):
        super().__init__(a, b, self.image.get_width() // 2, self.image.get_height() // 2, "Key")
        self.itemrect = pygame.Rect(self.image.get_rect())
...
def makedesks(floor, clsnum, a, b):
    ...
    if floor == 1: desklist = desklist1
    else: desklist = desklist2
    for i in range(4):
        for j in range(4):
            globals()[f"desk{floor}{clsnum}{i}{j}"] = ImageEnvironment(pygame.image.load('./img/Environments/desk.png'), a + 7*i, b + 7*j)
...
npc1 = NPC(humanImage, 290, 295, "Hello!", {"Axe": Bottle}, "Bye Bye")
desk = ImageEnvironment(pygame.image.load('./img/Environments/desk.png'), 300, 300)
desklist1 = []
for clsnum, a, b in [(2, 156, 315), (3, 216, 256), (4, 216, 315), (5, 336, 256), (6, 336, 315), (7, 396, 256), (8, 396, 315)]:
...
envs = [Lake({"Bottle": Axe}), npc1, desk]
screens[0].environments.append(npc1)
screens[1].environments.extend(desklist1)
for i in range(2, 5):
    screens[i].environments.extend(desklist2)

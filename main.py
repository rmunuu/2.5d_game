    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYUP:
            if event.key == K_ESCAPE:
                with open('./map/map_data.pkl', 'wb') as f:
        ...
        if event.type == KEYDOWN or event.type == KEYUP:
            if event.key == K_w:
                ...
                last = 0
                if len(map_data[checkx1][checky1])!=0:
                    last = map_data[checkx1][checky1][-1][1]+1
                map_data[checkx1][checky1].extend([(blockidx, last+i) for i in range(height)])
                ...
                map_data[checkx1][checky1] = map_data[checkx1][checky1][height:]
            elif event.key == K_DELETE and event.type == KEYUP:
                map_data[checkx1][checky1].clear()
            elif event.key == K_F1 and event.type == KEYUP: # 지붕 만들때 잠깐 쓸거임
                map_data[checkx1][checky1].extend([(blockidx, 2) for i in range(1)])

...
start_img = pygame.transform.scale(pygame.image.load('./img/start_img.png').convert_alpha(), (DISPLAYSURF.get_width(), DISPLAYSURF.get_height()))
end_img = pygame.transform.scale(pygame.image.load('./img/end_img.png').convert_alpha(), (DISPLAYSURF.get_width(), DISPLAYSURF.get_height()))
...
inputidimg = pygame.transform.scale(pygame.image.load('./img/inputidimg.png').convert_alpha(), (DISPLAYSURF.get_width(), DISPLAYSURF.get_height()))
...
class Screen:
    ...
    def makescreen(self):
            ...
            for j in range(2 * RENDER + 1):
                ...
                if i == RENDER and j == RENDER:
                    ...
                    if self.complete_num:
                        ...
                        DISPLAYSURF.blit(self.cplt_img, (DISPLAYSURF.get_rect().centerx-self.cplt_img.get_width()/2,
        ...
        self.text(f'{(pygame.time.get_ticks()-gamestarttime)//1000}', 30, DISPLAYSURF.get_rect().centerx + 685, DISPLAYSURF.get_rect().centery - 405)
        self.text('stage1', 30, DISPLAYSURF.get_rect().centerx + 685, DISPLAYSURF.get_rect().centery - 385)
        self.text(f'id:{inputidstr}', 30, DISPLAYSURF.get_rect().centerx + 685, DISPLAYSURF.get_rect().centery - 365)
...
class Human():
    def __init__(self):
        ...
        self.stx = DISPLAYSURF.get_rect().centerx-864+507
        self.sty = DISPLAYSURF.get_rect().centery-540+320
...
class Item:
    ...
    def activate(self, x, y):
        for env in screen.environments:
            if self.name in env.dic.keys() and env.envrect.left <= x <= env.envrect.right and env.envrect.top <= y <= env.envrect.bottom:
...
screen = screens[0]
lake = Environment(pygame.transform.scale(pygame.image.load('./img/Environments/img.png'), (500, 300)), 214, 189, {"Bottle" : Axe})
screens[0].environments.append(lake)
human = Human()
...
def inputid(): # inputidstr 이 최종 입력된 사용자 id
    global inputidstr
    sli = []
    det_inputid = 0
    ...
    while True:
        global screen
        # DISPLAYSURF.fill((100, 200, 200))
        DISPLAYSURF.blit(inputidimg, (0, 0))
        inputidstr = ''.join(sli)
        screen.text(inputidstr, 60, 500, 400)
...
def startgame():
    ...
    while True:
        ...
        DISPLAYSURF.blit(tutorial_img, (1000, 300))
        DISPLAYSURF.blit(play_img, (1000, 500))
...
def ending():
    ...
    while True:
        ...
        for i in range(len(idtime)):
            id, time = idtime[i]
            screen.text(f'{id} : {time}s', 30, DISPLAYSURF.get_rect().centerx - 100, DISPLAYSURF.get_rect().centery - 200 + 20*i)

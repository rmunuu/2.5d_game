...
def makescreen(posx, posy):
    ...
    for i in range(2 * RENDER + 1):  # for every row of the map...
        if i + int(posx) - RENDER < 0 or i + int(posx) - RENDER >= len(rooftop):
            continue
        for j in range(2 * RENDER + 1):
            if j + int(posy) - RENDER < 0 or j + int(posy) - RENDER >= len(rooftop[0]):
                continue
            if flip == 0:
                tile = rooftop[i + int(posx) - RENDER][j + int(posy) - RENDER]
            else:
                tile = rooftop[2 * int(posx)- (i + int(posx) - RENDER)][2 * int(posy) - (j + int(posy) - RENDER)]
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN or event.type == KEYUP:
            if event.key == K_w:
                ...
                last = 0
                if len(rooftop[checkx1][checky1])!=0:
                    last = rooftop[checkx1][checky1][-1][1]+1
                rooftop[checkx1][checky1].extend([(blockidx, last+i) for i in range(height)])
            elif event.key == K_BACKSPACE and event.type == KEYDOWN:
                for i in range(height):
                    if len(rooftop[checkx1][checky1]) != 0:
                        rooftop[checkx1][checky1].pop()
                ...
                rooftop[checkx1][checky1] = rooftop[checkx1][checky1][height:]
            elif event.key == K_DELETE and event.type == KEYUP:
                rooftop[checkx1][checky1].clear()

            elif event.key == K_F1 and event.type == KEYUP: # 지붕 만들때 잠깐 쓸거임
                rooftop[checkx1][checky1].extend([(blockidx, 2) for i in range(1)])
            elif event.key == K_F12 and event.type == KEYUP:
                a, b, c, d, h = map(int, input('a, b, c, d, h: ').rstrip().split())
                if h>0:
                    for i in range(a, b+1):
                        for j in range(c, d+1):
                            rooftop[i][j].extend([(blockidx, k) for k in range(h)])
                else:
                    for i in range(a, b+1):
                        for j in range(c, d+1):
                            for _ in range(-h): rooftop[i][j].pop()
                with open('./map/rooftop.pkl', 'wb') as f:
                    pickle.dump(rooftop, f, protocol=pickle.HIGHEST_PROTOCOL)

            elif event.key == K_F11 and event.type == KEYUP:
                for i in range(149, 452):
                    for j in [249, 250, 350, 351]:
                        rooftop[i][j].extend([(blockidx, 2+k) for k in range(1)])
                    
            elif event.key == K_F10 and event.type == KEYUP:
                for i in range(149, 452):
                    for j in [249, 250, 350, 351]:
                        for _ in range(1): rooftop[i][j].pop()

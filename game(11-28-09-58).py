                    ...
                    if self.complete_num:
                        ...
                        DISPLAYSURF.blit(self.cplt_img,
                                         (self.realx - self.cplt_img.get_width() / 2,
                                          self.realy - self.cplt_img.get_height()))
...
class LockMaker(ImageEnvironment):
    def __init__(self, a, b):
        super().__init__(pygame.transform.scale(pygame.image.load("./img/Environments/lock.png"), (200, 302)), a, b, 3,
                         {"KeyMaker": NoItem})
...
# doors.append(Door(4, 284, 315, 258, 258, 5, 265, 266, -1))
human = Human()
screen = screens[0]
waterflag = 1
desklist1 = []
...
def game_3():
    global envs, gamestarttime, inputid, humanImage
...
def random_generator():
    ...
    x, y = -1, -1
    while x == -1 or max(screens[idx].map_data[x][y], key=lambda x: x[1])[1] != 0 or (x, y) in screens[idx].environments.keys() \
...
def game_1():
    ...
    tree3 = ImageEnvironment(pygame.transform.scale(pygame.image.load('./img/Environments/envtree.png'), (300, 480)),
                             304, 188, 0, {"Axe": Tree})
    npc = NPC(pygame.transform.scale(pygame.image.load('./img/Environments/stu_6.png'), (100, 225)), 300, 300, "Find my 5 gems",
              {"Guntlet" : NoItem}, f"Your homebase password is {password}")
    envs = [Lake({"Bottle": WaterBottle, "Fishing": Fish}), tree1, tree2, tree3]
    screens[0].environments = {(283, 182): tree1, (311, 179): tree2, (304, 188): tree3}

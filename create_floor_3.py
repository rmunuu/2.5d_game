...
with open('./map/floor_3.pkl', 'rb') as f:
...
def makescreen(posx, posy):
    ...
    for i in range(2 * RENDER + 1):  # for every row of the map...
        if i + int(posx) - RENDER < 0 or i + int(posx) - RENDER >= len(floor_3):
            continue
        for j in range(2 * RENDER + 1):
            if j + int(posy) - RENDER < 0 or j + int(posy) - RENDER >= len(floor_3[0]):
                continue
            if flip == 0:
                tile = floor_3[i + int(posx) - RENDER][j + int(posy) - RENDER]
            else:
                tile = floor_3[2 * int(posx)- (i + int(posx) - RENDER)][2 * int(posy) - (j + int(posy) - RENDER)]
...
while True:
    for event in pygame.event.get():
        if event.type == QUIT:
            with open('./map/floor_3.pkl', 'wb') as f:
        ...
        if event.type == KEYDOWN or event.type == KEYUP:
            if event.key == K_w:
                ...
                last = 0
                if len(floor_3[checkx1][checky1])!=0:
                    last = floor_3[checkx1][checky1][-1][1]+1
                floor_3[checkx1][checky1].extend([(blockidx, last+i) for i in range(height)])
            elif event.key == K_BACKSPACE and event.type == KEYDOWN:
                for i in range(height):
                    if len(floor_3[checkx1][checky1]) != 0:
                        floor_3[checkx1][checky1].pop()
                ...
                floor_3[checkx1][checky1] = floor_3[checkx1][checky1][height:]
            elif event.key == K_DELETE and event.type == KEYUP:
                floor_3[checkx1][checky1].clear()

            elif event.key == K_F1 and event.type == KEYUP: # 지붕 만들때 잠깐 쓸거임
                floor_3[checkx1][checky1].extend([(blockidx, 2) for i in range(1)])
            elif event.key == K_F12 and event.type == KEYUP:
                a, b, c, d, h = map(int, input('a, b, c, d, h: ').rstrip().split())
                if h>0:
                    for i in range(a, b+1):
                        for j in range(c, d+1):
                            floor_3[i][j].extend([(blockidx, k) for k in range(h)])
                else:
                    for i in range(a, b+1):
                        for j in range(c, d+1):
                            for _ in range(-h): floor_3[i][j].pop()
                with open('./map/floor_3.pkl', 'wb') as f:
                    pickle.dump(floor_3, f, protocol=pickle.HIGHEST_PROTOCOL)
